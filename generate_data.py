...
def generate_agh_data(dataset_size, agh_size, fleet_size=4):
    n_hour = np.arange(10, 20)
    n_min = 60
    n_gate, prob = (91, np.load('problems/agh/arrival_prob.npy'))  # 登机口数、小时、分钟、到达概率
    print(prob)
    loc = 1 + np.random.choice(n_gate, size=(dataset_size, agh_size))  # 随机生成登机口索引 (1-91)
    arrival = (60 * np.random.choice(n_hour, size=(dataset_size, agh_size), p=prob)
               + np.random.randint(0, n_min, size=(dataset_size, agh_size)))  # 到达时间
    stay = torch.tensor([120, 120, 120]).repeat(dataset_size, 1)  # 停留时间（根据类型）
    type_ = torch.tensor(np.random.randint(0, 3, size=(dataset_size, agh_size)), dtype=torch.long)  # 节点类型 (0-2)
    ...
    # 按照概率逐行采样
    need = torch.stack([
        service_to_select[torch.multinomial(prob, agh_size, replacement=True)]
        for _ in range(dataset_size)
    ])  # 生成的数据是按照这个概率来的，因为50较小，所有实际的比例大小会上下波动

    return list(zip(loc.tolist(), arrival.tolist(), departure.tolist(), type_.tolist(), need.tolist()))


def generate_gaussian_agh_data(dataset_size, agh_size, fleet_size=4):
    n_hour = np.arange(10, 20)
    n_min = 60
    n_gate, prob = (91, np.load('./problems/agh/arrival_prob.npy'))  # 登机口数、小时、分钟、到达概率
    print(prob)
    prob = torch.softmax(torch.randn(10), dim=0).numpy()  # 保持高斯分布不变
    print(prob)
    ...
    # 按照概率逐行采样
    need = torch.stack([
        service_to_select[torch.multinomial(prob_need, agh_size, replacement=True)]
        for _ in range(dataset_size)
    ])  # 生成的数据是按照这个概率来的，因为50较小，所有实际的比例大小会上下波动
...
def generate_poisson_agh_data(dataset_size, agh_size, fleet_size=4):
    n_hour = np.arange(10, 20)
    n_min = 60
    n_gate, prob = (91, np.load('./problems/agh/arrival_prob.npy'))  # 登机口数、小时、分钟、到达概率
    print(prob)
    prob = np.random.poisson(4, 10)  # 保持泊松分布不变，但调整为10小时
    prob = prob / sum(prob)
    print(prob)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default='poisson_validation', help="Name to identify dataset")
